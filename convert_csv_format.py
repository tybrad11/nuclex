...
#NOTES
#0. You need to create NIDs for each term in the CSV first, after exporting from Google sheets
    #Use this to find blank rows: =IF(COUNTA(A2:S2) =0, 1,"")
#1. Make sure the csv paths are correct below
#2. Note that the reader skips a few lines, those are header values in the csv
#3. The first entry, RadLex entity, is handled differently. May need to fix.
#4. After running this, run conver_csv_to_owl.py. This one is run first
...
input_csv = os.path.join(r"C:\Users\tjb129\Downloads\NucLex & Common Data Elements - Theranostics - NucLex_v0.1_FROZEN.csv")
output_csv = os.path.join(r"C:\Users\tjb129\Downloads\NucLex_new_Oct2025.csv")

#for determining if term is in other ontology
path_radlex = os.path.join(r"\\onfnas01.uwhis.hosp.wisc.edu\radiology\Research\Bradshaw\Tyler\nuclex\radlex_owl\Radlex_separate_synonyms.xls")
path_nci =    os.path.join(r"\\onfnas01.uwhis.hosp.wisc.edu\radiology\Research\Bradshaw\Tyler\nuclex\nci_thesaurus_owl\Thesaurus_curated.csv")
...
with open(input_csv, 'r',encoding='utf-8') as infile, open(output_csv, 'w', newline='') as outfile:
        ...
        # Write header row
        writer.writerow(['NucLex_NID', 'Preferred_name', 'Parent_class_id', 'Parent_class_name', 'RadLex_RID', 'NCI_Thesaurus_Code','UMLS_CUI'])
        ...
        parent_classes_name = ['RadLex entity', '1','2', '3','4', '5','6','7', '8']
        parent_classes_id = ['NID000001', '1','2', '3','4', '5','6','7', '8']
