...
import config #get api_key

def query_pmc_and_save_as_text_file(topic, n_years, write_dir):
    #topic = NM or RAD
    #This function will query PMC for the journals listed below and save them as .txt files in the write_dir. You need
    #own eutils api_key stored in config.py
    ...
    if topic == 'NM':
        ...
        raise ValueError('topic parameter should be NM or RAD')

    for journal_query in journals:
        for j in range(2023-n_years,2023):
            query_date = str(j)
            ...
            params = {
                ...
                'retmax': retmax,
                'retmode': 'xml',
                'api_key': api_key #store in config.py
            }
            ...
            output_dir = write_dir + journal_query.replace(' ','_') + query_date
...
#main
write_dir = '/mnt/tjb129/Bradshaw2/Tyler/nuclex/nuc_med_articles/from_pmc/'
for topic in ['NM', 'RAD']:
    query_pmc_and_save_as_text_file(topic, n_years=5, write_dir=write_dir)